...
// Изначально предполагается, что эта работа выполнена для пользователя, который знает язык C хотя бы на начальном уровне и разбирается во встроенных библиотеках данного языка, его синтаксисе
...
// - Чтобы сопоставлять дамп пакета в соответствии с приведенными структурами, необходимо преобразовать сетевой пакет в шестнадцатеричный дамп. Выводить информацию о пакете мы будем в виде шестнадцатеричных значений
...
// Вывод информации о сетевом пакете, далее вы увидите тот же самый процесс, по сути это и есть обработка перехваченного пакета
    ...
    // Определяем тип протокола исходя из структуры Ethernet заголовка
    if (eth->type[0] == 0x08 && eth->type[1] == 0x00) { // IPv4
        ...
        // Обработка пакетов протоколов TCP, UDP, ICMP и IGMP. Протокол пакета определяется из структуры IP заголовка
	...
	// Прописываем аналогично функцию обработки для остальных протоколов
        ...
        printf("Unknown Protocol Type: %02x%02x\n", eth->type[0], eth->type[1]); // Пакет имеет протокол, который не разбирается в данной в функции, не проверяется соответственно
    }
}
...
int main() {
    ...
    while (choice < 0) {
        ...
        if (device != NULL) {
            ...
            if (handle == NULL) {
                fprintf(stderr, "Не удалось открыть интерфейс %s: %s\n", device->name, errbuf); // Ошибка при открытии выбранного интерфейса
                return 1; 
            }

            // Перехватываем первые 10 пакетов, отправляем их на функцию обработки packet_handler(), описанную выше, см. подробнее в прикрепленных ресурсах
