...
#include <stdlib.h>
#include <string.h>
#include <pcap.h> // Основная интересующая нас библиотека libpcap, она выполняет практически всю работу в нашем коде
#include <arpa/inet.h>
...
// - Чтобы поставить данные пакета в соответствие с приведенными структурами, необходимо преобразовать сетевой пакет в шестнадцатеричный дамп. Выводить информацию о пакете мы будем в виде шестнадцатеричных значений
...
// Функция обработки пакетов. Программа проходит по шестнадцатеричному дампу пакета, выводя данные о заголовках исходя из указанных выше структур и длины дампа пакета. см. подробнее в прикрепленных ресурсах
...
// Вывод информации о сетевом пакете. Далее вы увидите тот же процесс, по сути, это и есть обработка перехваченного пакета
