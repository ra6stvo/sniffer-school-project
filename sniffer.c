...
void process_packet(const struct pcap_pkthdr *header, const u_char *packet) {
    ...
    if (eth->type[0] == 0x08 && eth->type[1] == 0x00) { // IPv4
        ...
        if (ip->protocol == 6) { // TCP
            struct tcp_header *tcp = (struct tcp_header *)(packet + sizeof(struct ethernet_header) + sizeof(struct ip_header));
            printf("TCP Header:\n");
            printf("  Source Port: ");
        ...
        printf("Unknown Protocol Type: %02x%02x\n", eth->type[0], eth->type[1]); // Пакет, который имеет протокол, который не разбирается в данной в функции, не проверяется соответственно
    }
}
